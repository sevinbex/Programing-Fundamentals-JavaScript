function vacation(input) {
  ...
  while (input[0] != undefined) {
    if (currentExpenses > totalBudget) {
      ...
      console.log(
        `Not enough money to continue the trip. You need ${remaining.toFixed(
          2
        )}$ more.`
      );
      return;
    ...
    if (
      (currentDay % 3 == 0 && currentDay != 0) ||
      (currentDay % 5 == 0 && currentDay != 0)
    ) {
  ...
  console.log(
    `You have reached the destination. You have ${remaining.toFixed(
      2
    )}$ budget left.`
  );
}
