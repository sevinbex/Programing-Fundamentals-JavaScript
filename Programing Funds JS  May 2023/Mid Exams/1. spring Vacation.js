function foo(arr) {
  ...
  expenses +=
    foodPerPerson * people * days +
    roomPerPerson * people * days * hotelDiscount;
  for (let i = 0; i < days; i++) {
    expenses += arr[i] * fuelPerKmPrice;
    if (exceed(budget, expenses))
      return `Not enough money to continue the trip. You need ${(
        expenses - budget
      ).toFixed(2)}$ more.`;
  ...
  return `You have reached the destination. You have ${(
    budget - expenses
