function foo(args) {
  ...
  function shoot(target) {
    const pointsGained =
      field[target] > 5 && field[target] !== 0 ? 5 : field[target];
    field[target] =
      field[target] > 5 && field[target] !== 0 ? field[target] - 5 : 0;
    points += pointsGained;
  }
  ...
  function shootRight(index, length) {
    ...
    if (length + index > field.length - 1)
      shoot(getOutboundIndex.right(length, index, field));
    else {
      shoot(index + length);
    }
  }
